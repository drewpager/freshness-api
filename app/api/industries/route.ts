...
export async function GET() {
  try {
    ...
    const industriesSheetID: number | undefined = Number(process.env.GOOGLE_ISID);

    if (!industriesSheetID) {
      throw new Error('GOOGLE_ISID is not defined');
    }
    ...
    const sheet = doc.sheetsById[industriesSheetID ? industriesSheetID : 598151405];
