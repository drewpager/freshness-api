...
export async function GET() {
  try {
    ...
    const websitesSheetID: number | undefined = Number(process.env.GOOGLE_WSID);

    if (!websitesSheetID) {
      throw new Error('GOOGLE_WSID is not defined');
    }
    ...
    const sheet = doc.sheetsById[websitesSheetID ? websitesSheetID : 1234613033];
